...
branchModule.deactivate = function( $branchNode ) {
    ...
    if ( $branchNode.is( '.question' ) ) {
         ...
         */
        const dataEl = this.form.model.node( name, index ).getElement();
            ...
            value = Array.prototype.slice.call( dataEl.querySelectorAll( '*' ) )
                .filter( el => {
                    if ( el.children.length === 0 ) {
                        const path = that.form.model.getXPath( el, 'instance' );
                        const n = that.form.view.html.querySelector( `.calculation > [name="${path}"], .or-appearance-dn > [name="${path}"]` );
                        return !n;
                    }
                    return false;
                } )
