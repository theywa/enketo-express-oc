...
function addSignedStatus( form ) {
    ...
    if ( signature ) {
        const statusEl = range.createContextualFragment( `<div class="record-signed-status">${signature.replace( /\\n/g, '<br/>' ).replace( /\n/g, '<br/>' )}</div>` );
        ...
        const _changeHandler = ev  =>{
            if ( !ev.target.closest( '.or-appearance-dn' ) ){
                ...
                event.currentTarget.removeEventListener( events.XFormsValueChanged().type, _changeHandler );
            }
        };

        form.view.html.addEventListener( events.XFormsValueChanged().type,_changeHandler );
    }
}
